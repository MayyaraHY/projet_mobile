...
import 'contracts_for_car_screen.dart';
import 'contract_create_screen.dart'; // Import the contract creation screen
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Spacer for bottom nav bar (match compact height)
              ...
              children: [
                ...
                Expanded(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Text('Price', style: TextStyle(fontSize: 11, color: Colors.black54)),
                      const SizedBox(height: 2),
                      FittedBox(fit: BoxFit.scaleDown, child: Text(voiture.prixFormate, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold))),
                    ],
                  ),
                ),
                    ...
                    children: [
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () {
                            Navigator.push(context, MaterialPageRoute(builder: (context) => PlanifierRendezVousScreen(voiture: voiture)));
                          },
                          style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 6), backgroundColor: Colors.blue[600], foregroundColor: Colors.white, textStyle: const TextStyle(fontSize: 10)),
                      ...
                      Expanded(
                        child: OutlinedButton(
                          onPressed: () {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => ContractsForCarScreen(matricule: voiture.matricule)));
                          },
                          style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 6), side: const BorderSide(color: Colors.black54)),
                          child: const Text('Contracts', style: TextStyle(fontSize: 10, color: Colors.black)),
                        ),
                      ),
                      const SizedBox(width: 6),
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () async {
                            final res = await Navigator.push(context, MaterialPageRoute(builder: (_) => ContractCreateScreen(voiture: voiture)));
                            if (res == true) {
                              if (!context.mounted) return;
                              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Contract created')));
                            }
                          },
                          style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 6), backgroundColor: Colors.green[700], foregroundColor: Colors.white, textStyle: const TextStyle(fontSize: 10)),
