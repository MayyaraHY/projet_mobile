...
import '../screens/rendez_vous_list_screen.dart';
...
class _VoitureListScreenState extends State<VoitureListScreen> {
  ...
  // View & sort state
  bool _isGrid = true;
  String _sortMode = 'newest'; // 'newest', 'price_asc', 'price_desc'
  ...
  void _applySort() {
    if (_sortMode == 'newest') {
      _voitures.sort((a, b) => b.annee.compareTo(a.annee));
    } else if (_sortMode == 'price_asc') {
      _voitures.sort((a, b) => a.prix.compareTo(b.prix));
    } else if (_sortMode == 'price_desc') {
      _voitures.sort((a, b) => b.prix.compareTo(a.prix));
    }
  }
      ...
      appBar: AppBar(
        title: _isSearching
            ? TextField(
                ...
                autofocus: true,
                decoration: const InputDecoration(hintText: 'Search by matricule, brand or model'),
                textInputAction: TextInputAction.search,
                onSubmitted: (value) async {
                  if (value.trim().isEmpty) {
                    _loadVoitures();
                  } else {
                    await _searchVoitures(value.trim());
                  }
                },
              )
         ...
         actions: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (ctx) => [
              ...
              const PopupMenuItem(value: 'price_asc', child: Text('Sort: Price ↑')),
              const PopupMenuItem(value: 'price_desc', child: Text('Sort: Price ↓')),
            ],
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const RendezVousListScreen(),
                ),
              );
            },
            tooltip: 'Rendez-vous',
            ...
            onSelected: (value) async {
              if (value == 'backup') {
                try {
                  ...
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Database backed up to: $path')));
                } catch (e) {
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Backup failed: $e')));
                }
              } else if (value == 'delete_all') {
                final ok = await _showConfirmDialog('Delete all cars?', 'This will permanently delete ALL cars from the database. Are you sure?');
                if (ok == true) {
                  try {
                    final deleted = await _voitureService.deleteAllVoitures();
                    if (deleted) {
                      _showSuccessSnackBar('All cars deleted');
                      _loadVoitures();
                    } else {
                      _showErrorSnackBar('No cars deleted');
                    }
                  } catch (e) {
                ...
                // reload key after returning
                try { await BadWordService.loadRuntimeApiKey(); } catch (_) {}
               }
            },
            itemBuilder: (ctx) => [
              const PopupMenuItem(value: 'backup', child: Text('Backup database')),
              const PopupMenuItem(value: 'delete_all', child: Text('Delete all cars', style: TextStyle(color: Colors.red))),
            ...
            child: _isLoading
                ...
                : _voitures.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          final v = _voitures[index];
                          return ListTile(
                            leading: v.hasImage ? Image.file(File(v.image!), width: 64, height: 48, fit: BoxFit.cover) : const Icon(Icons.directions_car),
                            title: Text('${v.marque} ${v.modele}'),
                            subtitle: Text('${v.matricule} • ${v.prixFormate}'),
                            onTap: () => _showVoitureDetails(v),
                            onLongPress: () => _showAdminActionsSheet(v),
                          );
                        },
                      ),
          ),
  ...
  // Export the currently displayed voitures to CSV and return the file path
  Future<String> _exportCsv() async {
    final list = _voitures;
    if (list.isEmpty) throw Exception('No cars to export');

    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/voitures_export_${DateTime.now().millisecondsSinceEpoch}.csv');

    final buffer = StringBuffer();
    buffer.writeln('matricule,marque,modele,annee,puissance,cylindres,carburant,kilometrage,prix,description,image');
    for (final v in list) {
      final safe = (String? s) => (s == null) ? '' : s.replaceAll('"', '""');
      buffer.writeln('"${v.matricule}","${v.marque}","${v.modele}",${v.annee},${v.puissance},${v.cylindres},"${v.carburant}",${v.kilometrage},${v.prix},"${safe(v.description)}","${safe(v.image)}"');
    }
  ...
  // Backup the app database file to application documents directory and return the backup path
  Future<String> _backupDatabase() async {
    final dbPath = await getDatabasesPath();
    final src = p.join(dbPath, DatabaseConstants.databaseName);
    final srcFile = File(src);
    if (!await srcFile.exists()) throw Exception('Database file not found at $src');

    final dir = await getApplicationDocumentsDirectory();
    final dest = p.join(dir.path, 'backup_${DateTime.now().toIso8601String().replaceAll(':', '-')}.db');
  ...
  Future<void> _saveVoiture() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        try {
          puissance = double.parse(_puissanceController.text.trim());
        } catch (e) {
          throw Exception('Invalid power value');
        }
        try {
          cylindres = int.parse(_cylindresController.text.trim());
        } catch (e) {
          throw Exception('Invalid cylinders value');
        }
        try {
          kilometrage = double.parse(_kilometrageController.text.trim());
        } catch (e) {
          throw Exception('Invalid kilometrage value');
        }
        try {
