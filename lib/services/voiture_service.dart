...
class VoitureService {
  ...
  Future<Voiture> createVoiture({
    ...
    final existing = await _repository.getVoitureByMatricule(matricule);
    if (existing != null) {
      throw Exception('Une voiture avec le matricule $matricule existe déjà');
    }
  ...
  Future<List<Voiture>> getAllVoitures({String? ownerId}) async {
    final voitures = await _repository.getAllVoitures();
    if (ownerId != null) {
      return voitures.where((v) => v.ownerId == ownerId).toList();
    }
    return voitures;
  }
  ...
  Future<List<Voiture>> getVoituresByUser(String userId) async {
    if (userId.trim().isEmpty) {
      throw Exception('L\'ID de l\'utilisateur ne peut pas être vide');
    }
    return (await getAllVoitures()).where((v) => v.userId == userId).toList();
  }
